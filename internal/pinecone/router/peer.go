...
func (p *peer) stop(err error) {
	...
	p.router.state.Act(nil, func() {
		...
		// Log the disconnection in a user-friendly way
		peerID := p.public.String()
		if len(peerID) > 8 {
			peerID = peerID[:8] + "..."
		}
		...
		if err != nil {
			// 简化错误信息，只显示关键部分
			errorMsg := err.Error()
			if strings.Contains(errorMsg, "forcibly closed") {
				p.router.log.Printf("🔌 节点 %s 连接断开 (端口:%d)", peerID, p.port)
			} else if strings.Contains(errorMsg, "timeout") {
				p.router.log.Printf("⏰ 节点 %s 连接超时 (端口:%d)", peerID, p.port)
			} else {
				p.router.log.Printf("❌ 节点 %s 连接异常 (端口:%d): %s", peerID, p.port, errorMsg)
			}
		} else {
			p.router.log.Printf("✅ 节点 %s 正常断开 (端口:%d)", peerID, p.port)
		}
	})
}
...
func (p *peer) _write() {
	...
	// 检查peer是否仍然活跃，避免无限递归调用
	if p.started.Load() {
		// 使用非阻塞方式调度下一次写入，避免goroutine堆积
		select {
		case <-p.context.Done():
			// 上下文已取消，停止递归
			return
