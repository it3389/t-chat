...
// 增加超时时间以提高连接稳定性
const peerKeepaliveTimeout = time.Second * 30
...
// 优化：从1分钟减少到30秒，加快坐标缓存更新
const coordsCacheLifetime = time.Second * 30

// coordsCacheMaintainInterval is how often we will clean
// out stale entries from the coords cache.
// 优化：从1分钟减少到15秒，提高缓存清理频率
const coordsCacheMaintainInterval = time.Second * 15
