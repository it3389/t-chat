...
// tcpOptions 适配 net.ListenConfig/ net.Dialer 的 Control 回调（占位实现）
func (m *Multicast) tcpOptions(network, address string, c syscall.RawConn) error {
    // 可在此处使用 RawConn.Control 设置底层套接字选项
    return nil
}

// udpOptions 适配 net.ListenConfig 的 Control 回调（占位实现）
func (m *Multicast) udpOptions(network, address string, c syscall.RawConn) error {
    // 可在此处使用 RawConn.Control 设置底层套接字选项
    return nil
}
